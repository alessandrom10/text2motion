...
def main(args = None, cond_dict = None):
    ...
    cond_dict["Tyranno"]["real_texts"] = texts
    ...
    for rep_i in range(args.num_repetitions):
        ...
        for i, motion in enumerate(sample):
            real_text = model_kwargs['y']["real_text"][i] if 'real_text' in model_kwargs['y'] else None
            ...
            mp4_name = '%s_'%(object_type)+real_text+'_#%d.mp4'%(len(existing_mp4_files))
...
def create_condition(object_types, cond_dict, n_frames, temporal_window, t5_conditioner, max_joints, feature_len):
    ...
    for object_type in object_types:
        batch=list()
         # motion, m_length, parents, joints_perm, inv_joints_perm, tpos_first_frame, offsets, self.temporal_mask_template, joints_graph_dist, joints_relations, object_type, joints_names, text_embed
        ...
        if isinstance(embed_text, list):
            index = np.random.randint(len(embed_text))
            embed_text = embed_text[index]
            real_text = real_text[index]
