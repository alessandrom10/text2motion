...
def rotate_point_cloud(batch_data):
    """ Randomly rotate the point clouds to augment the dataset.
        Rotation is applied per shape along all three axes (x, y, z).
    ...
    B, N, C = batch_data.shape
    rotation_angles_batch = np.zeros((B, 3), dtype=np.float32)

    for k in range(B):
        rotation_angle_x = np.random.uniform(0, 2 * np.pi)
        rotation_angle_y = np.random.uniform(0, 2 * np.pi)
        rotation_angle_z = np.random.uniform(0, 2 * np.pi)

        rotation_angles_batch[k, 0] = rotation_angle_x
        rotation_angles_batch[k, 1] = rotation_angle_y
        rotation_angles_batch[k, 2] = rotation_angle_z
        
        cos_x, sin_x = np.cos(rotation_angle_x), np.sin(rotation_angle_x)
        cos_y, sin_y = np.cos(rotation_angle_y), np.sin(rotation_angle_y)
        cos_z, sin_z = np.cos(rotation_angle_z), np.sin(rotation_angle_z)
