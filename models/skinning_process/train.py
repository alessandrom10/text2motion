...
from dataset.dataset_animals import DatasetAnimals
...
def main(args):
    ...
    # create model
    model = skinnet(args.nearest_bone, args.Dg, args.Lf)
    model.to(device)
    ...
    train_loader = DataLoader(DatasetAnimals(root=args.train_folder), batch_size=args.train_batch, shuffle=True)
    val_loader = DataLoader(DatasetAnimals(root=args.val_folder), batch_size=args.test_batch, shuffle=False)
    test_loader = DataLoader(DatasetAnimals(root=args.test_folder), batch_size=args.test_batch, shuffle=False)
    if args.evaluate:
        print('\nEvaluation only')
